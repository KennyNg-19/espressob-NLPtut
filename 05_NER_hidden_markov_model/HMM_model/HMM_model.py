...
class HMM_NER:
    ...
    def fit(self, train_dic_path):
        ...
        # take the logarithm，乘法 -> 加法，取log【防止概率过小的数，计算结果下溢】
    ...
    def estimate_transition_and_initial_probs(self, train_dic):
        ...
        for dic in tqdm(train_dic):
            for i, tag in enumerate(dic["label"][:-1]):
                ...
                next_tag = self.tag2idx[dic["label"][i + 1]]  # 找到相邻两个标签
    ...
    def estimate_emission_probs(self, train_dic):
        ...
        self.emission[self.emission == 0] = self.epsilon  # 防止log 0,
...
if __name__ == '__main__':
    model = HMM_NER(
        char2idx_path="./dicts/char2idx.json", tag2idx_path="./dicts/tag2idx.json"
    )
